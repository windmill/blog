...
function RSSImport($display,$feedurl,$displaydescriptions,$truncatetitle,$newwindow,$displayfeedname) {

    $string = "<!-- start of RSS feed content by InlineFeed plugin v2.01 - http://kruyt.org -->\n";
		...
		$string .= wptexturize("<br /><b><a href=" . $rss->channel['link'] . ">" . $rss->channel['title'] . ": " . $rss->channel['description'] . "</a></b><br />");
				...
				foreach ($rss->items as $item) {
								...
								if( is_numeric($truncatetitle) ){
										if(strlen($item['title'])>$truncatetitle)
												{
...
function inlinefeed_func($atts) {
	...
	return RSSImport($display,$rss_feed_url,$displaydescriptions,$truncatetitle,$newwindow,$displayfeedname);
}
